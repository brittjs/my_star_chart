...
module.exports = {
  ...
  getAllFriendsForUser: function* getAllFriendsForUser(next) {
    ...
    var userFriends = yield db.sequelize.query("SELECT * FROM \"Users\" WHERE \"id\" in (SELECT \"Friend1Id\" FROM \"Friendships\" WHERE \"Friend2Id\"=$[this.state.user.id] UNION SELECT \"Friend2Id\" FROM \"Friendships\" WHERE \"Friend1Id\"=$[this.state.user.id]", { type: db.sequelize.QueryTypes.SELECT});
